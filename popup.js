...
//   const toggle = document.getElementById("readefineMasterToggle");
...
//     if (toggle.checked) {
//       // Inject content.js which adds the TTS speaker button
//       chrome.scripting.executeScript({
//         target: { tabId: tab.id },
//         files: ["content.js"]
//       });
//     } else {
//       // Remove the TTS speaker button
//       chrome.scripting.executeScript({
//         target: { tabId: tab.id },
//         func: () => {
//           const btn = document.getElementById("readefine-speaker-button");
//           if (btn) btn.remove();
//         }
//       });
//     }
...
document.addEventListener('DOMContentLoaded', () => {
  chrome.storage.sync.get([
    ...
    // Always inject speaker on popup load if enabled
    if (tab?.id && masterToggle.checked) {
      chrome.scripting.executeScript({
        target: { tabId: tab.id },
        files: ["content.js"]
      });
    ...
    masterToggle.addEventListener("change", async () => {
      ...
      chrome.storage.sync.set({ readefineEnabled: enabled });
